...
object ExampleServiceActor {
  def props(model: ActorRef)(implicit askTimeout: Timeout): Props = Props(classOf[ExampleServiceActor], model, askTimeout)
  def name = "service"
}

class ExampleServiceActor(model: ActorRef, implicit val askTimeout: Timeout) extends Actor with ExampleService {
  def actorRefFactory = context
  def receive = runRoute(route(model))
}

trait ExampleService extends HttpService with ServiceJsonProtocol {
          ...
          term =>
            ...
            onSuccess(model ? msg) {
              case ItemSummaries(summaries) =>
                ...
                complete(OK, CacheHeader(maxAge), summaries map {
                  PublicItemSummary(_)
                })
            }
        }
      } ~
        path("items" / IntNumber) {
          id =>
            onSuccess(model ? id) {
              case item: Item =>
